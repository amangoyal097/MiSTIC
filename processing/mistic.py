...
def mark_directions(count, nbhd):
    ...
    for i in range(1, len(count)):
        ...
        for j in range(0, len(nb)):
            diffs.append(count[nb[j]] - count[i])
...
def process_year(year, vectorLayer, location, neighbors):
    ...
    directions = mark_directions(values, neighbors)
...
def mistic(filePath, startYear, endYear, numYears):
    vectorDs = ogr.Open(filePath, gdalconst.GA_ReadOnly)
    vectorLayer = vectorDs.GetLayer()

    location = getLocation(filePath)
    neighbors = shp_neighbors(filePath)

    for year in range(startYear, endYear + 1):
        foci, zone, lat, long = process_year(
            year, vectorLayer, location, neighbors)
        plot_foci(filePath, foci, zone, lat, long)
        break


mistic("../shapefile/district.shp", 2001, 2010, 10)
