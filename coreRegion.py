...
from driver import num_of_Years, num_of_Units, nbhd, foci, valid_foci, loc, zone_Units
...
def convert_percent_to_time(min_sup_frac, itf):
    """
...
def classify_params(max_prune_value, min_freq_value):
    """
    ...
    min_freq_value = [floor((value / 100.0) * num_of_Years) for value in min_freq_value]
...
def find_ref_foci(itf_value, itf, output_Path, ref_foci):
    """
...
def merge_adjacent_foci(mode, ref_foci):
    """
    ...
    for tuples in sorted_rF:
        ...
        if polygon in ref_foci:
            ...
            common = list(set(n) & set(ref_foci))  # find intersection with ref_foci
            for element in common:
                ref_foci.remove(element)  # remove them from ref_foci


def find_core_zones(ccInfo, ref_foci, core_zone_influence):
    ...
    for i in range(len(ref_foci)):
        ...
        for j in range(num_of_Years):
            ...
            focus = ccInfo[i][j]
            if focus:
                zone_of_focus = zone_elements[focus]
...
def init(numFoci, crInfo, crFoci):
...
def find_cc(itf_value, min_sup_value, ref_foci, all_foci_frequency):
    """Find Info about CC"""
    ...
    for i, focus in enumerate(ref_foci):
        ...
        for j in range(num_of_Years):
            ...
            if ccPrev in vF:
                ...
                ccLoc = loc[ccPrev - 1]  # ccLoc is [X Y]
                n = nbhd[ccPrev - 1]  # n is list of neighbors
    ...
    print("ccInfo length = ", len(ccInfo))
    print("reffoci length = ", len(ref_foci))
    return ccInfo, ccFoci


def find_cc_old(numFoci, ccInfo, ccFoci, ref_foci):
    """
    Find Info about CC
            ...
            if ccPrev in vF:
                ...
                if I:
                    ...
                    if len(I) > 1:
                        ...
                        val = min(ccDist)
                        ind = ccDist.index(val)
                    ccInfo[i][j] = I[ind]
                    ccPrev = I[ind]
            if not bool(ccFoci[i]):  # can be removed
                ccFoci[i][ccInfo[i][j]] = 1
            else:
                # Search for ccInfo[i][j] in ccFoci[i]
                if ccInfo[i][j] in ccFoci[i]:
                    ccFoci[i][ccInfo[i][j]] += 1
                else:
                    ccFoci[i][ccInfo[i][j]] = 1
...
def write_data_cc(
    ...
    core_type_names = ["core type"] + [core_classify_name[i] for i in core_classify_cc]
    with open(filename, "w") as fp:
        writer = csv.writer(fp, delimiter=",")
        writer.writerow(header)
...
def find_cr(numFoci, crInfo, crFoci, ref_foci):
    """Find Info about CR"""
    ...
    for i in range(numFoci):
        ...
        for j in range(num_of_Years):
            ...
            if current_rF in vF:
                ...
                crLoc = loc[current_rF - 1]  # crLoc is [X Y]
                n = nbhd[current_rF - 1]  # n is list of neighbors
                temp = n
                ...
                for k in range(len(n)):
                    temp = list(set(temp) | set(nbhd[n[k] - 1]))
...
def write_data_cr(
    ...
    core_type_names = ["core type"] + [core_classify_name[i] for i in core_classify_cr]
    with open(filename, "w") as fp:
        writer = csv.writer(fp, delimiter=",")
        writer.writerow(header)
...
def core_analysis(
    ...
    max_prune_value, min_freq_value = classify_params(max_Prune, min_Freq)
    print(max_prune_value)
    print(min_freq_value)

    # Get minimum support value in time units
    min_sup_value, itf_value = convert_percent_to_time(min_Support, itf)
    ...
    if mode == 0:
        ...
        ccInfo, ccFoci = find_cc(
            itf_value, min_sup_value, ref_foci_cc, all_foci_frequency
        )
        print(len(ref_foci_cc))
